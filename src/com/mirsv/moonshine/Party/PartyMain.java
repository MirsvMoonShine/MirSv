					...
					if(getParty(p.getUniqueId()) != null) {
						for(UUID u: getParty(p.getUniqueId()).getPlayers()) {
							if(Bukkit.getOfflinePlayer(u).isOnline()) {
								Bukkit.getPlayer(u).sendMessage("[" + ChatColor.LIGHT_PURPLE + "PC" + ChatColor.WHITE + "][" + ChatColor.DARK_PURPLE + getParty(p.getUniqueId()).getPartyName() + ChatColor.WHITE + "] " + Prefix + p.getName() + ": " + ChatColor.LIGHT_PURPLE + Message);
							}
						}
					}
					else {
	...
	public void onChat(AsyncPlayerChatEvent event) {
		...
		if (getConfig().getBoolean("enable.Party", true) && chat.contains(p.getUniqueId())) {
			...
			if (getParty(p.getUniqueId()) != null) {
				...
				event.getPlayer().sendMessage(prefix + ChatColor.YELLOW + "당신은 파티에 소속되어있지 않습니다. /g 명령어를 통해 전체 채팅 모드로 변경해주세요!");
			}
		}
	}
	@EventHandler(priority = EventPriority.HIGHEST)
